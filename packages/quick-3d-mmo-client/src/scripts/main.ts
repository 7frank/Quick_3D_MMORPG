...
import {
  Constants,
  Defs,
  EntityManager as aEntityManager,
  SpatialHashGrid as aSpatialHashGrid,
} from 'quick-3d-mmo-shared'
...
const {
  ...
  EntityManager,
  SpatialHashGrid,
} = { ...Constants, ...Defs, ...aEntityManager, ...aSpatialHashGrid }
...
class CrappyMMOAttempt {
  ...
  LoadControllers_() {
    ...
    // Hack
    this.scene_ = threejs.GetComponent(
      NAMED_COMPONENTS.THREEJS_CONTROLLER,
    )?.scene_
    this.camera_ = threejs.GetComponent(
      NAMED_COMPONENTS.THREEJS_CONTROLLER,
    )?.camera_
    this.threejs_ = threejs.GetComponent(
      NAMED_COMPONENTS.THREEJS_CONTROLLER,
    )?.threejs_
    const ui = new Entity()
    ...
    for (let k in WEAPONS_DATA) {
      database
        .GetComponent(KNOWN_ENTITIES.INVENTORY_DATABASE_CONTROLLER)
        .AddItem(k, WEAPONS_DATA[k])
    }
  }
